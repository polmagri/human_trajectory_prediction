...
def calculate_3d(x, y, depth_image, intrinsics, window_size=10):
    if x <= 10 or y <= 10 or x >= 1270 or y >= 710:
    ...
    pixel = [x, y]
    depth = min_depth / 1000.0  # Convert depth to meters
...
# Load the intrinsic parameters from the provided data
intrinsics = rs.intrinsics()
intrinsics.width = 1280
intrinsics.height = 720
intrinsics.ppx = 644.119
intrinsics.ppy = 363.619
intrinsics.fx = 645.682
intrinsics.fy = 645.682
intrinsics.model = rs.distortion.none  # Assuming no distortion
intrinsics.coeffs = [0, 0, 0, 0, 0]  # Assuming no distortion coefficients
...
# Create the axis for depth image visualization
...
# Parameters for iterating over frames
frame_count = 0
max_frames = 2500  # Set a maximum number of frames to process
...
# Configure the RealSense pipeline with frame queue size set to 2
...
# List all files in the directories# Start the pipeline with the specified configuration
...
try:
    for rgb_file, depth_file in zip(rgb_files, depth_files):
        ...
        try:
            ...
            color_image = np.load(rgb_file)
        except Exception as e:
            print(f"Errore nel caricamento dei frame: {e}")
            break

        # Normalize the depth image for visualization
        depth_image_vis = cv2.normalize(depth_image, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
                ...
                if hasattr(result, 'keypoints'):
                    ...
                    for i, (x, y) in enumerate(zip(keypoints_list[::2], keypoints_list[1::2])):
                        ...
                        if label:
                            ...
                            x_3d, y_3d, z_3d, min_depth = calculate_3d(int(x), int(y), depth_image, intrinsics)
                    ...
                    if 'Hip.L' in keypoints_2d and 'Hip.R' in keypoints_2d:
                        ...
                        if hip_l != (0, 0) and hip_r != (0, 0):
                            ...
                            x_3d, y_3d, z_3d, min_depth = calculate_3d(pelvis_x, pelvis_y, depth_image, intrinsics)
                            if not np.isnan(z_3d) and (x_3d != 0 or y_3d != 0 or z_3d != 0):
                    ...
                    if 'Shoulder.L' in keypoints_2d and 'Shoulder.R' in keypoints_2d:
                        ...
                        if shoulder_l != (0, 0) and shoulder_r != (0, 0):
                            ...
                            x_3d, y_3d, z_3d, min_depth = calculate_3d(neck_x, neck_y, depth_image, intrinsics)
                            if not np.isnan(z_3d) and (x_3d != 0 or y_3d != 0 or z_3d != 0):
                    ...
                    if 'Eye.L' in [kp[0] for kp in keypoints_3d] and 'Eye.R' in [kp[0] for kp in keypoints_3d] and 'Neck' in [kp[0] for kp in keypoints_3d]:
                        ...
                        if orientation_head_degrees is not None:
                            head_orientation_degrees = angleX_camera_neck_degrees + orientation_head_degrees
                            print(f"head orientation: {head_orientation_degrees} degrees")
                        else:
        ...
        # Plot the depth image on the second subplot
        ax_depth.clear()
        ax_depth.imshow(depth_image_vis, cmap='gray')
        ...
        rgb_video_writer.write(cv2.cvtColor(color_image, cv2.COLOR_RGB2BGR))
        depth_video_writer.write(depth_image_vis)
