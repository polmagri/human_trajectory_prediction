...
# Function to read raw depth data
...
depth_vis_path = f'/home/mohamed/ros2_ws/dataset/depth/{file_number}_depth_vis'
...
depth_vis_files = sorted([f for f in os.listdir(depth_vis_path) if f.endswith('.jpg')])

try:
    for frame_idx, (color_file, depth_raw_file, depth_vis_file) in enumerate(zip(color_files, depth_raw_files, depth_vis_files)):
        ...
        depth_vis_image = cv2.imread(os.path.join(depth_vis_path, depth_vis_file))
        ...
        cv2.imshow('Depth Visualization', depth_vis_image)
